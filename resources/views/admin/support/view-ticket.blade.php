                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    <h5 class="my-0 lh-base">
                                        <a href="javascript:void(0)" class="text-reset">{{ $ticket->user->name }}</a>
                                    </h5>
                    ...
                    <div>
                        <div id="chat-scroll-container" class="chat-scroll p-3" data-simplebar>
                            <ul class="chat-list" data-apps-chat="messages-list">
                                @php
                                $isAdminViewing = auth()->user()->role === 'admin';
                                @endphp

                                @foreach($ticket->messages->sortBy('created_at') as $index => $message)
                                @php
                                $isAdminMessage = $message->is_admin;
                                ...
                                // Admin viewing: admin messages on right
                                // User viewing: user messages on right
                                $showAsRight = ($isAdminViewing && $isAdminMessage) || (!$isAdminViewing && !$isAdminMessage);
                                $chatClass = $showAsRight ? 'chat-group odd' : 'chat-group';

                                // Name tag
                                $senderLabel = $showAsRight ? 'You' : ($isAdminMessage ? 'Support' : $ticket->user->name);
                                @endphp

                                <li class="{{ $chatClass }}" id="{{ $showAsRight ? 'odd' : 'even' }}-{{ $index }}">
                                    ...
                                    <div class="chat-body">
                                        <div>
                                            <h6 class="d-inline-flex">{{ $senderLabel }}</h6>
                        ...
                        <div class="p-3 border-top position-sticky bottom-form-chat bottom-0 w-100 mb-0 bg-light">
                            <form class="row align-items-center g-2" action="{{ route('admin.ticket.message', $ticket->reference_id) }}" method="POST" enctype="multipart/form-data" id="chat-form">
                                @csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (chatForm) {
            ...
            function appendNewMessage(data) {
                ...
                // Determine alignment
                const chatClass = 'chat-group odd'; // Admin is viewing, so admin should be right side
        ...
        <div class="chat-body">
            <div>
                <h6 class="d-inline-flex">${data.sender_name}</h6>
    ...
    `;
                ...
                // Bind attachment preview modal to new button
                if (data.attachment_path) {
                    const button = li.querySelector('.view-attachment-btn');
                    if (button) {
        ...
        document.querySelectorAll('.close-ticket-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "You are about to close this ticket. The user won't be able to send more messages.",
                    ...
                    if (result.isConfirmed) {
                        fetch(`/admin/ticket/${referenceId}/close`, {
                                method: 'POST',
                                headers: {
