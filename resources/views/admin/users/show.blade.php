...
@section('content')
                            ...
                            <div class="row d-flex justify-content-center align-items-center">
                                ...
                                <div class="col-md-5">
                                    <div class="d-flex align-items-center gap-3 mb-4">
                                        <img src="{{ $viewedUser->profile && $viewedUser->profile->profile_picture ? asset('storage/profile_pictures/' . $viewedUser->profile->profile_picture) : asset('dashboard_assets/assets/images/users/user-avatar.jpg') }}"
                                            alt="Profile"
                                            class="avatar-xl rounded-circle border border-light border-2 profile">
                                        <div>
                                            <h4 class="text-dark fw-medium">{{ $viewedUser->name }}</h4>
                                            <span
                                                class="badge {{ $viewedUser->active ? 'bg-success' : 'bg-danger' }} px-2 py-1 fs-12">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <div class="mt-3">
                                            <div class="d-flex align-items-center gap-2 mb-2">
                                                ...
                                                <p class="mb-0 text-dark me-1">{{ $viewedUser->email }}
                                                    <a href="{{ route('admin.users.email.form', $viewedUser->id) }}"
                                ...
                                <div class="col-md-7  wallet-details">
                                    ...
                                    <div class="d-flex align-items-center mb-4">
                                        ...
                                        <div class="ms-auto">
                                            <span
                                                class="badge p-1 {{ $viewedUser->wallet ? ($viewedUser->wallet->status ? 'bg-success' : 'bg-danger') : 'bg-secondary' }}">
                                    ...
                                    <div class="d-flex justify-content-center align-items-center gap-3 mt-4">
                                        ...
                                        @if ($viewedUser->wallet)
                                            <div>
                                                @if ($viewedUser->wallet->status)
                                                    <form
                                                        action="{{ route('admin.users.wallet.deactivate', $viewedUser->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <button type="submit"
                                                            class="btn btn-danger w-100 wallet-action-btn deactivate-wallet-btn">
                                                            <i class="ti ti-ban me-1"></i> Deactivate Wallet
                                                        </button>
                                                    </form>
                                                @else
                                                    <form
                                                        action="{{ route('admin.users.wallet.activate', $viewedUser->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <button type="submit"
                                                            class="btn btn-success w-100 wallet-action-btn activate-wallet-btn">
                                        ...
                                        <div>
                                            @if ($viewedUser->active)
                                                <form action="{{ route('admin.users.ban', $viewedUser->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="btn btn-danger w-100 user-action-btn ban-user-btn">
                                                        <i class="ti ti-ban me-1"></i> Ban User
                                                    </button>
                                                </form>
                                            @else
                                                <form action="{{ route('admin.users.unban', $viewedUser->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="btn btn-success w-100 user-action-btn unban-user-btn">
                ...
                <div class="col-md-12 mt-2">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-primary p-1">Total Referrals:
                                {{ $viewedUser->referrals->count() }}</span>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="border-end pe-3">
                                        ...
                                        @if ($viewedUser->referred_by)
                                            ...
                                            <div class="mt-2">
                                                <a href="{{ route('admin.users.show', $referrer->id) }}"
                                                    class="btn btn-sm btn-outline-primary">
                                                    View Profile
                                                </a>
                                            </div>
                                                            ...
                                                            <td>
                                                                <div class="d-flex align-items-center gap-2">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <small
                                                                            class="text-muted">{{ $referral->email }}</small>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td class="text-end">
    ...
    <div class="modal fade" id="fundWalletModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form id="fundWalletForm" action="{{ route('admin.users.fund-wallet', $viewedUser->id) }}"
                    method="POST">
                    @csrf
    ...
    <div class="modal fade" id="deductWalletModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                @if ($viewedUser->wallet)
                    <form id="deductWalletForm" action="{{ route('admin.users.deduct-wallet', $viewedUser->id) }}"
                        method="POST">
                        @csrf
                        <div class="modal-header">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <span class="input-group-text">Max:
                                        ${{ number_format($viewedUser->wallet->balance, 2) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
