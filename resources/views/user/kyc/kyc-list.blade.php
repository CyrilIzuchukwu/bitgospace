        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="text-center mb-4">
                    <h3 class="mb-2 section-title" style="text-transform: uppercase;">KYC Verification History</h3>
                    <p class="text-muted w-100 m-auto">
                        Overview of all your KYC verification requests
                    </p>
                </div>
                ...
                <div class="no-investment">
                    <div class="not-found card">
                        ...
                        <div class="text-notfound">
                            ...
                            <span class="text-gray-100">You haven't submitted any KYC verification requests yet.</span>
                        </div>
                    </div>
                </div>
                @else
                                    ...
                                    <table class="table table-borderless table-hover table-custom table-nowrap align-middle mb-0">
                                        <thead class="bg-light bg-opacity-50 thead-sm">
                                            <tr class="text-uppercase fs-10">
                                                ...
                                                <th scope="col" class="text-muted text-center">Rejection Reason</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($kycs as $kyc)
                                            <tr data-status="{{ strtolower($kyc->status) }}">
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($kyc->submitted_at)->format('M j, Y') }}</td>
                                                <td>
                                                    @if($kyc->reviewed_at)
                                                    {{ \Carbon\Carbon::parse($kyc->reviewed_at)->format('M j, Y') }}
                                                    @else
                                                    <span class="text-muted">-</span>
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    <button class="btn btn-sm btn-outline-primary">{{ $kyc->rejection_reason ?? '-' }}</button>
                                                </td>
                                            </tr>
                                            @endforeach
                                        </tbody>
